...
def init_db():
    ...
    cursor = conn.cursor()
    cursor.execute("Create table if not exists  Product_Items(id integer primary key autoincrement,name text,price integer)")
    cursor.close()
    conn.close()
init_db()
...
def Save_Records():
    ...
    if(nm=="" and pr == ""):
        ...
        conn = sq.connect("my.db")
        cursor = conn.cursor();
        cursor.execute("insert into Product_Items(name,price)values(?,?)",(nm,pr))
        conn.commit()
        cursor.close()
        conn.close()
        tree.insert("",0,values=(cursor.lastrowid,nm,pr))
        e1.delete(0,END)
        e2.delete(0,END) 
...
def delete_record():
    ...
    if(get_id["values"]==""):
        ...
        conn = sq.connect("my.db")
        cursor = conn.cursor();
        cursor.execute("delete from Product_Items where id=?",(record_id,))
        conn.commit()
        cursor.close()
        conn.close()
...
def update_record():
    ...
    if(row["values"] == ""):
        ...
        def rec_update():
            ...
            conn = sq.connect("my.db")
            cursor = conn.cursor();
            cursor.execute("update Product_Items set name=?,price=? where id=?",(nm2,pr2,id1))
            conn.commit()
            cursor.close()
            conn.close()
